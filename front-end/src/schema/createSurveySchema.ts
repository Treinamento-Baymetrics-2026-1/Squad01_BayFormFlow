...
export const surveyFormSchema1 = z
  .object({
    ...
    prazoEncerramento: z
      .string()
      .min(1, "A data de encerramento é obrigatória"),
  })
  .refine(
    (data) => {
      if (!data.prazoInicio || !data.prazoEncerramento) {
        return true;
      }
    ...
    {
      message:
        "O prazo de encerramento não pode ser menor que o prazo de início",
